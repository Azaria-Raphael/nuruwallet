...
class BudgetScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final prevSpendingAsync = ref.watch(previousMonthSpendingProvider);
    ...
    return Scaffold(
      ...
      body: categoriesAsync.when(
        data: (categories) {
          ...
          final prevSpending = prevSpendingAsync.valueOrNull ?? {};

          double effectiveLimit(CategoryEntry cat) {
            if (cat.monthlyLimit <= 0) return 0;
            if (!cat.rolloverEnabled) return cat.monthlyLimit;
            final prevSpent = prevSpending[cat.id] ?? 0.0;
            final unused = (cat.monthlyLimit - prevSpent).clamp(0.0, double.infinity);
            return cat.monthlyLimit + unused;
          }
            ...
            children: [
              ...
              for (final cat in categories) ...[
                _CategoryBudgetCard(
                  ...
                  effectiveLimit: effectiveLimit(cat),
                ),
...
class _CategoryBudgetCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final cat = category;
    final hasLimit = effectiveLimit > 0;
    final progress = hasLimit ? (spent / effectiveLimit).clamp(0.0, 1.0) : 0.0;
    final isOver = hasLimit && spent > effectiveLimit;
    final catColor = Color(cat.colorValue);
    final hasRollover =
        cat.rolloverEnabled && cat.monthlyLimit > 0 && effectiveLimit > cat.monthlyLimit;
              ...
              Row(
                children: [
                  ...
                  if (isOver)
                    ...
                    Text(
                      hasLimit
                          ? '${CurrencyFormatter.formatCompact(spent)} / ${CurrencyFormatter.formatCompact(effectiveLimit)}'
                          : spent > 0
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      content: Column(
        ...
        children: [
          Text(
            'Set the maximum you want to spend on '
            '${widget.category.name} this month.',
            style: Theme.of(context).textTheme.bodySmall?.copyWith(
              color: Theme.of(
                context,
